...
SWEP.Contact        = ""
SWEP.Purpose        = "The IFAK's purpose is to treat soldier's injuries."
	...
	function SWEP:Draw3D2DCamera()
		...
		cam.Start3D2D(pos + ang:Right() * -3.5 + ang:Forward(), ang, 0.015 * GetConVarNumber("fas2_textsize"))
			...
			if am > 0 then
				...
				draw.ShadowText("+40 HP", "FAS2_HUD24", -20, -60, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
			else
				am = self.Owner:GetAmmoCount("Quikclots")
	...
	function SWEP:DrawHUD()
		...
		if GetConVarNumber("fas2_customhud") <= 0 or self.Owner:ShouldDrawLocalPlayer() then
			...
			if am > 0 then
				...
				draw.ShadowText("+40 HP", "FAS2_HUD24", x2 + 100, y2 + 75, Green, Black, 2, TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT)
			else
				am = self.Owner:GetAmmoCount("Quikclots")
